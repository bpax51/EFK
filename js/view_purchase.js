	...
	rowselection();
	 $("input:checkbox").change(function() {
        ...
        function confirmSubmit() {
            var agree = confirm("Voulez-vous vraiment supprimer la ligne selectionnée ?");
            if (agree)
                return true;
            else
                return false;
...
function confirmDeleteSubmit() {
	   ...
	   var movies2 = JSON.parse(retrievedData);
		  ...
		  // alert(flag);
            ...
            if (flag ==0) {
                alert("Vous devez choisir une ligne !");
                return false;
            } else {
                var agree = confirm("Voulez-vous vraiment supprimer les lignes selectionnées ?");
                if (agree)
{
	var file1="viewpurchase";
 $.ajax({
